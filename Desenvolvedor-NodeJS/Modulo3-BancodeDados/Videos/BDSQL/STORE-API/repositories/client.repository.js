...
async function insertClient(client) {
    ...
    try {
        ...
        const values = [client.nome, client.cpf, client.phone, client.email, client.address]
...
async function updateClient(client) {
    const conn = await connect()
    try {
        //const sql = "UPDATE clients SET (name, cpf, phone, email, address) VALUES ($1, $2, $3, $4, $5) RETURNING client_id"
        const sql = "UPDATE clients SET name = $1, cpf=$2, phone=$3, email=$4, address=$5 WHERE client_id=$6 RETURNING *"
        const values = [client.nome, client.cpf, client.phone, client.email, client.address, client.client_id]
        const res = await conn.query(sql, values)
